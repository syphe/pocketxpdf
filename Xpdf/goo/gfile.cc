...
#if defined(WIN32)
  //---------- Win32 ----------
  //wchar_t *w = new wchar_t[128];
  //GetTempPath(255,w);
	static int id = 0;
	char *s = new char[256];
	sprintf(s,"\\Temp\\tmpfont%d",id++);
	//WideCharToMultiByte(CP_ACP,WC_SEPCHARS,w,-1,s,256,0,0);
...
#else
  return gFalse;//system(cmd) ? gFalse : gTrue;
#endif
}
...
#ifdef WIN32
    fa = GetFileAttributes(s->getCStringW());
...
GDir::GDir(char *name, GBool doStatA) {
  ...
  tmp = path->copy();
  tmp->append("/*.*");
  hnd = FindFirstFile(tmp->getCStringW(), &ffd);
  delete tmp;
#elif defined(ACORN)
#elif defined(MACOS)
#else
...
#if defined(WIN32)
  if (hnd) {
	  char *ascii = new char[wcslen(ffd.cFileName)];
	WideCharToMultiByte(CP_ACP,WC_SEPCHARS,ffd.cFileName,-1,ascii,wcslen(ffd.cFileName),0,0);
    e = new GDirEntry(path->getCString(), ascii, doStat);
	delete[]ascii;
