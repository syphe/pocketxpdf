...
FileStream::FileStream(char *fileNameA, Guint startA, GBool limitedA,
		       Guint lengthA, Object *dictA):
    BaseStream(dictA) {
  filename = new char[strlen(fileNameA)+1];
  memcpy(filename,fileNameA,strlen(fileNameA)+1);
  f = fopen(filename,"rb");
...
Stream *FileStream::makeSubStream(Guint startA, GBool limitedA,
				  Guint lengthA, Object *dictA) {
  return new FileStream(filename, startA, limitedA, lengthA, dictA);
}
...
GBool FileStream::fillBuf() {
  ...
  if (ferror(f)!=0){
	  ...
	  //MessageBoxW(NULL,L"File Handle lost, retrying...",L"Error",MB_ICONERROR);
	  fclose(f);
	  f = fopen(filename,"rb");
	  fseek(f, bufPos, SEEK_SET);
	  n = fread(buf, 1, n, f);
	  if (ferror(f)!=0){
		  MessageBoxW(NULL,L"Read Error",L"Error",MB_ICONERROR);
	  }
  }
  bufEnd = buf + n;
  if (bufPtr >= bufEnd) {
...
GString *CCITTFaxStream::getPSFilter(int psLevel, char *indent) {
  ...
  printf(s1, 50, "/Columns %d ", columns);
  s->append(s1);
  if (rows != 0) {
    printf(s1,50, "/Rows %d ", rows);
    s->append(s1);
  }
  if (!endOfBlock) {
