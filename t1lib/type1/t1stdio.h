...
typedef struct F_FILE {
  ...
  FILE     *fd;         /* underlying file descriptor */
} F_FILE;
...
extern F_FILE *T1Open(), *T1eexec();
extern int T1Close(F_FILE *), T1Ungetc(int,F_FILE *), T1Read(), T1Getc(F_FILE *);

#define  tfclose(f)          T1Close(f)
#define  tfopen(name,mode)   T1Open(name,mode)
#define  tungetc(c,f)        T1Ungetc(c,f)
#define  tfgetc(f)           T1Getc(f)
#define  tfread(bufP,size,n,f) T1Read(bufP,size,n,f)
#define  tfeof(f)            (((f)->flags & FIOEOF) && ((f)->b_cnt==0))
#define  tferror(f)          (((f)->flags & FIOERROR)?(f)->error:0)
