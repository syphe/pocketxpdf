...
#  define O_BINARY 0x0
  ...
  if ((of->fd=fopen(fn, "rb")) < 0) return NULL;

  /* We check for pfa/pfb file */
  if (fread( &c,sizeof(char), 1,of->fd)!=1) {
    fclose( of->fd);
    return(NULL);
  }
  else
	   ...
	   F_FILE *f) /* Read a line */
{
  ...
  while ( size>0) {
    ...
    if ( (Decrypt==0) &&
	 ((eexecP=(char *)strstr( string, "eexec"))!=NULL) ) {
		 ...
		 F_FILE *f)
{
  ...
  off_save=fseek( f->fd, 0, SEEK_CUR);
  if ((buf=(char *)malloc( size+1))==NULL ) {
	  //Sys_PrintF("mem allocation error: %s:%d", __FILE__, __LINE__);
    return( -1);
  }
  fseek( f->fd, -size, SEEK_END);
  fread(buf, sizeof(char),size,f->fd);
  ...
  while ((j--)>-1) {
    ...
    if ((ctmP=(char *)strstr( &(buf[j]), "cleartomark"))!=NULL) {
...
unsigned long T1GetFileSize( F_FILE *f) 
{
  ...
  off_save=fseek( f->fd, 0, SEEK_CUR);
  filesize=fseek( f->fd, 0, SEEK_END);
  fseek( f->fd, off_save, SEEK_SET);
  return( filesize);
}
  ...
  F_FILE *f;         /* Stream descriptor */
{
  ...
  */
  ...
  randomP[0] = tgetc(f);  /* store first non white space char */
  tfread(randomP+1, 1, 3, f);  /* read 3 more, for a total of 4 */
  ...
  if (asc) {  /* ASCII form, convert first eight bytes to binary */
    tfread(randomP+4, 1, 4, f);  /* Need four more */
  ...
  F_FILE *f;         /* Stream descriptor */
{
  ...
  if (starthex80){ /* we have a pfb-file -> be aware of pfb-blocks */
    if ( pfbblocklen-accu >= F_BUFSIZ){
      /* fill the buffer */
      rc = fread(f->b_base,sizeof(char), F_BUFSIZ,f->fd);
      bytecnt+=rc;
      accu +=rc;
    }
    else{
	...
	rc = fread(f->b_base,sizeof(char), pfbblocklen-accu,f->fd);
	bytecnt +=rc;
	accu +=rc;
	/* ... and examine the next header */
	i=fread(hdr_buf,sizeof(char), 6,f->fd);
	bytecnt +=i;
	pfbblocklen=0;
	...
	if (pfbblocklen-accu >= F_BUFSIZ){
	  ...
	  rc = fread(f->b_base,sizeof(char), pfbblocklen,f->fd);
	  accu +=rc;
	}
      }
    }
