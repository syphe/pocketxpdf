...
extern int fprintf(FILE *f, char *text, ...);
extern int vfprintf(FILE *f, char *text, va_list);
...
void *T1_InitLib( int log)
{
  ...
  /* Check environment variable ENV_LOG_STRING. By this means, a user may
     ...
     no effect! */
  if (0){//(envlogreq=getenv(ENV_LOG_STRING))!=NULL) {
...
#  define O_BINARY 0x0
  ...
  i=fread( filebuffer,sizeof(char), filesize,fd);
  fclose(fd);   /* Close font database file */
...
//	  Sys_PrintF("mem allocation error: %s:%d", __FILE__, __LINE__);
  ...
  i=fread(  filebuffer,sizeof(char), filesize,fd);
  fclose(fd);   /* Close XLFD font database file */
